...
def load_scaled(image_path, screen_width=None, screen_height=None, verbose=False, screen_mode="fill"):
    # Decode and rescale once; every other function reuses this result
    ...
    if screen_width is not None and screen_height is not None:
        ...
        # INTER_AREA: correct and much faster than LANCZOS4 when downscaling
...
def find_largest_region(img, verbose=False, stride=2, threshold=100.0, aspect_ratio=1.0, horizontal_padding=50, vertical_padding=50):
...
def get_dominant_color(img, x, y, w, h):
    ...
    # Subsample: dominant color is stable well below every pixel
    if region.shape[0] > 20000:
        region = region[::region.shape[0] // 20000 + 1]
    ...
    _, labels, centers = cv2.kmeans(region, K, None, criteria, 3, cv2.KMEANS_RANDOM_CENTERS)
...
def main():
    ...
    if args.largest_region:
        ...
        if center:
            ...
            x1 = cx - region_w // 2
            y1 = cy - region_h // 2
            dominant_color = get_dominant_color(img, x1, y1, region_w, region_h)
    ...
    # Optional grid of dominant colors tiling the whole screen: lets the QML side
    # pick a locally-accurate color instantly as widgets move, no re-runs needed.
    grid_extra = {}
    if args.color_grid_cols > 0 and args.color_grid_rows > 0:
        gh, gw = img.shape[:2]
        cw = max(1, round(gw / args.color_grid_cols))
        ch = max(1, round(gh / args.color_grid_rows))
        colors = []
        for row in range(args.color_grid_rows):
            for col in range(args.color_grid_cols):
